...
## Unsupervised LSTM-autoencoder health indicators

`scripts/run_cmapss_lstm_ae.py` trains one conditional LSTM autoencoder for
each of FD001--FD004. It fits preprocessing and model weights using only
cycles 1--50 from the training engines, passes all 3 operating conditions to
the encoder/decoder, reconstructs all 21 sensors, and uses the endpoint
reconstruction error of a rolling 20-cycle window as the health indicator.
Sensor and operating-condition inputs are min–max normalized to `[0, 1]` using
only the first 50 training cycles. The plotted `hi_0_1` is then calibrated
using only the healthy training median and the training 95th-percentile error,
and clipped to `[0, 1]` for the same visual scale as the paper's Figure 10.

Run the complete C-MAPSS experiment with:

```bash
conda run -n llm python scripts/run_cmapss_lstm_ae.py \
  --fds FD001 FD002 FD003 FD004 \
  --data-dir dataset/CMAPSSData \
  --output-dir outputs/CMAPSS/lstm_autoencoder
```

Each FD directory contains `hi_train.csv`, `hi_test.csv`, `hi_curves.png`,
`model.pt`, scalers, training history, and metadata. The plotted
`hi_mae` is the raw reconstruction error, `hi_relative_to_healthy` is retained
for diagnostics, and `hi_0_1`/`hi_0_1_rolling_mean` are the normalized plotting
columns.

### N-CMAPSS paper-style method transferred to C-MAPSS

`scripts/run_cmapss_paper_lstm_ae.py` transfers the paper architecture to
C-MAPSS. It trains one model per FD subset using the previously selected 14
sensors (`s2,s3,s4,s7,s8,s9,s11,s12,s13,s14,s15,s17,s20,s21`) and the three
operating conditions. Because C-MAPSS has no `hs` field, training cycles 1--50
are used as the healthy stage. Inputs are min-max normalized to `[-1,1]`; the
raw paper-style HI is `hi_sum_14`, while `hi_0_1` is a healthy-training-based
normalization that is zeroed at the healthy baseline but is not upper-clipped.

```bash
conda run -n llm python scripts/run_cmapss_paper_lstm_ae.py \
  --data-dir dataset/CMAPSSData \
  --output-dir outputs/CMAPSS/lstm_autoencoder_paper_transfer
```

The output contains independent FD001--FD004 models, per-engine HI CSV files,
per-FD figures, and `hi_curves_all_fds.png`.

To align the generated HI with C-MAPSS ground-truth RUL and plot their
relationship, run:

```bash
conda run -n llm python scripts/plot_cmapss_hi_rul.py
```

This writes `hi_rul_train.csv` and `hi_rul_test.csv` for every FD, using exact
full-trajectory RUL for training engines and the supplied `RUL_FDxxx.txt`
endpoint values for test engines. It also writes per-FD same-engine overlay
figures, `hi_vs_true_rul_scatter.png`, and `correlation_summary.csv`.

### Paper-faithful N-CMAPSS DS02 replay

The paper's N-CMAPSS experiment is implemented separately in
`scripts/reproduce_n_cmapss_lstm_ae_paper.py`. It uses DS02-006, minute-level
aggregation, the Table 1 sensor set, min–max normalization to `[-1, 1]`,
healthy flights marked by `hs == 1`, local Luong attention with `D=5`, and the
Table 3 nine-sensor flight-level HI.

```bash
conda run -n llm python scripts/reproduce_n_cmapss_lstm_ae_paper.py \
  --data-file dataset/N-CMAPSS/N-CMAPSS_DS02-006.h5 \
  --output-dir outputs/N-CMAPSS/lstm_autoencoder_paper/DS02-006
```

To plot the HI generated by this trained model for every development and test
engine, run:

```bash
conda run -n llm python scripts/plot_n_cmapss_all_engine_hi.py
```

The results are saved under
`outputs/N-CMAPSS/lstm_autoencoder_paper/DS02-006/all_engine_hi/`, including
`all_engines_dev_test_hi.png`, an individual-engine faceted figure, and the
combined `all_engine_hi.csv`.

To apply the trained DS02-006 model to every readable N-CMAPSS dataset file,
including all dev/test engines, run:

```bash
conda run -n llm python scripts/score_n_cmapss_all_datasets_with_trained_model.py
```

The all-dataset outputs are written to
`outputs/N-CMAPSS/lstm_autoencoder_paper/DS02-006/all_datasets_all_engines/`.
The damaged/truncated `N-CMAPSS_DS08d-010.h5` file is recorded in
`skipped_datasets.json` and cannot be read until the source file is repaired.

### Condition-normalized N-CMAPSS LHI

To normalize the model HI across operating-condition classes, run:

```bash
conda run -n llm python scripts/compute_n_cmapss_lhi.py
```

For each dataset and `flight_class`, `B` is the median HI of flights with
`hs == 1`. The normalized indicator is
`LHI = log((HI + epsilon) / (B + epsilon))`; raw HI is retained in the output.
The combined CSV, per-dataset CSVs, baselines, and figures are saved under
`outputs/N-CMAPSS/lstm_autoencoder_paper/DS02-006/all_datasets_all_engines/lhi/`.
