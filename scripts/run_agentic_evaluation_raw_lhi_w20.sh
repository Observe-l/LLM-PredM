...
# Periodic evaluator experiment using leakage-free, unrolled LHI.
# No policy counterfactual evaluation and no canary deployment are used.
...
for fd in "${fds[@]}"; do
    ...
    echo "[$(date --iso-8601=seconds)] Starting $fd with raw lhi_rmse and evaluator W=20"
    "$python_bin" -m src.RAG_cmapss.joint_simulation \
        --fds "$fd" \
        --lhi_dir "$lhi_dir" \
        --output_dir "outputs/CMAPSS/RAG/agentic_evaluation_w20/$fd" \
        --score_col lhi_rmse \
